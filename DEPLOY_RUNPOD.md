...
**2026-08-07 real upgrade**: cs-careeradvisor moved from a 7B custom fine-tune
...
Ollama/llama.cpp — see Modelfile.cs-careeradvisor's comment for the full real
...
considered first. cs-careerbriefing + cs-careerreasoning + cs-embed stay the same small
...
- GPU: a real **40-80GB single-GPU pod (A100/H100-class)** — the old
  RTX A4000 16GB recommendation still doesn't fit once cs-careeradvisor needs
  ...
  cs-careeradvisor plus cs-careerbriefing/cs-careerreasoning/cs-embed resident together should fit
...
- Container disk: **60GB+** (down from the 70B model's 80GB+ recommendation
  — cs-careeradvisor's ~19GB estimated download plus the other 3 models plus real
...
cs-careerbriefing, cs-careerreasoning, and cs-embed are custom fine-tunes with no public
registry — there's no `ollama pull cs-careerreasoning`. From your **local
...
cs-careeradvisor excluded — see 3a below):
...
### 3a. Pull cs-careeradvisor directly on the pod (real 32B multilingual model)

Unlike the 3 custom fine-tunes above, cs-careeradvisor is now `aya-expanse:32b` —
a real, public Ollama library model, chosen specifically for genuine
23-language coverage (see Modelfile.cs-careeradvisor for the full real research
...
a home connection) and tag it as `cs-careeradvisor` using the repo's real Modelfile:

```bash
ollama pull aya-expanse:32b   # ~19GB estimated — real download, budget real time for this
cd /workspace/careercamp-ai   # or wherever cs_fixed/models/Modelfile.cs-careeradvisor was cloned
ollama create cs-careeradvisor -f Modelfile.cs-careeradvisor
```
...
Still confirm with `nvidia-smi` after cs-careeradvisor loads rather than assuming
...
curl -X POST <your-tunnel-url>/v1/chat/completions \
  ...
  -d '{"model":"cs-careerreasoning","messages":[{"role":"user","content":"hi"}]}'
```
