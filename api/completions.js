...
router.post('/', async (req, res) => {
  ...
  // cs-careeradvisor (careerlm-xl) is a fundamentally different tier from
  // sonnet/haiku: 32B, partial-CPU-offload on this hardware (see
  // models/Modelfile.cs-careeradvisor), real measured cold-start ~57s just to
  ...
  // real num_ctx (8192, from Modelfile.cs-careeradvisor) instead of the
  // model-agnostic 2048 floor ollamaInfer() falls back to when a caller
  // doesn't specify one.
  const isOpusTier = model === 'cs-careeradvisor' || model === 'careerlm-xl';
  const opts = {
