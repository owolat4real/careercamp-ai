...
const LOCAL_MODELS = {
  'cs-careerbriefing':      process.env.CS_HAIKU_MODEL  || 'cs-careerbriefing',
  'cs-careerreasoning':     process.env.CS_SONNET_MODEL || 'cs-careerreasoning',
  'cs-careeradvisor':       process.env.CS_OPUS_MODEL   || 'cs-careeradvisor',
...
async function callOllama(ollamaModel, messages, maxTokens, task, numCtx, wantsJson = false) {
  ...
  const body = {
    ...
    // should rarely fire, but 2048 (the smallest real ceiling, cs-careerbriefing's)
...
async function callWithRetry(feature, messages, numCtx) {
  ...
  const sonnetModel  = LOCAL_MODELS['cs-careerreasoning'];
  ...
  try {
    ...
    if (feature.model === 'cs-careerbriefing') {
      const leak = _DL(content);
      if (leak.leaked) {
        console.warn(`[CAMP] cs-careerbriefing leaked (${leak.reason}) → retrying cs-careerreasoning`);
        const retried = await callOllama(sonnetModel, messages, Math.max(feature.maxTokens, 800), feature.task, numCtx, wantsJson);
        return { content: retried, model: 'cs-careerreasoning', retriedFrom: 'cs-careerbriefing' };
      }
    }
    return { content, model: feature.model };
  } catch (primaryErr) {
    console.warn(`[CAMP] ${feature.model} failed:`, primaryErr.message?.slice(0, 60));
    if (feature.model !== 'cs-careerreasoning') {
      try {
        const fallback = await callOllama(sonnetModel, messages, Math.max(feature.maxTokens, 800), feature.task, numCtx, wantsJson);
        return { content: fallback, model: 'cs-careerreasoning', retriedFrom: feature.model };
      } catch (sonnetErr) {
        console.warn('[CAMP] cs-careerreasoning also failed:', sonnetErr.message?.slice(0, 60));
      }
    }
...
async function callRace(feature, messages, numCtx) {
  const haikuCall = callOllama(LOCAL_MODELS['cs-careerbriefing'], messages, feature.maxTokens, feature.task, numCtx, !!feature.schema)
    .then(content => {
      const leak = _DL(content);
      if (leak.leaked) throw new Error('haiku leaked');
      return { content, model: 'cs-careerbriefing' };
    });
...
async function localInfer({ userInput, task, maxTokens }) {
  ...
  try {
    const text = await callOllama(LOCAL_MODELS['cs-careerbriefing'], messages, maxTokens || 150, task);
    return { content: text };
  } catch {
    try {
      const text = await callOllama(LOCAL_MODELS['cs-careerreasoning'], messages, maxTokens || 300, task);
      return { content: text };
    } catch {
      return { content: '' };
    }
...
router.post('/:featureId', apiKeyGuard, async (req, res) => {
  ...
  const isHaikuTier = feature.model === 'cs-careerbriefing';
  ...
  if (!isSalary && feature.model === 'cs-careerreasoning' && _RR(feature.task)) {
  ...
  if (wantsStream && feature.streaming) {
    ...
    const isHaiku     = feature.model === 'cs-careerbriefing';
    ...
    try {
      for await (const event of streamLocal(ollamaModel, builtMessages, feature.maxTokens, feature.task, isHaiku, tierCfg.numCtx)) {
        if (event.type === 'leak_abort') {
          ...
          res.write(`data: ${JSON.stringify({ type: 'model_switch', to: 'cs-careerreasoning' })}\n\n`);
          for await (const e2 of streamLocal(LOCAL_MODELS['cs-careerreasoning'], builtMessages, Math.max(feature.maxTokens, 800), feature.task, false, tierCfg.numCtx)) {
