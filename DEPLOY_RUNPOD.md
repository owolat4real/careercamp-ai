...
**2026-08-07 real upgrade**: cs-careerking moved from a 7B custom fine-tune
...
Ollama/llama.cpp — see Modelfile.cs-careerking's comment for the full real
...
considered first. cs-careerchief + cs-careerprince + cs-embed stay the same small
...
- GPU: a real **40-80GB single-GPU pod (A100/H100-class)** — the old
  RTX A4000 16GB recommendation still doesn't fit once cs-careerking needs
  ...
  cs-careerking plus cs-careerchief/cs-careerprince/cs-embed resident together should fit
...
- Container disk: **60GB+** (down from the 70B model's 80GB+ recommendation
  — cs-careerking's ~19GB estimated download plus the other 3 models plus real
...
cs-careerchief, cs-careerprince, and cs-embed are custom fine-tunes with no public
registry — there's no `ollama pull cs-careerprince`. From your **local
...
cs-careerking excluded — see 3a below):
...
### 3a. Pull cs-careerking directly on the pod (real 32B multilingual model)

Unlike the 3 custom fine-tunes above, cs-careerking is now `aya-expanse:32b` —
a real, public Ollama library model, chosen specifically for genuine
23-language coverage (see Modelfile.cs-careerking for the full real research
...
a home connection) and tag it as `cs-careerking` using the repo's real Modelfile:

```bash
ollama pull aya-expanse:32b   # ~19GB estimated — real download, budget real time for this
cd /workspace/careercamp-ai   # or wherever cs_fixed/models/Modelfile.cs-careerking was cloned
ollama create cs-careerking -f Modelfile.cs-careerking
```
...
Still confirm with `nvidia-smi` after cs-careerking loads rather than assuming
...
curl -X POST <your-tunnel-url>/v1/chat/completions \
  ...
  -d '{"model":"cs-careerprince","messages":[{"role":"user","content":"hi"}]}'
```
